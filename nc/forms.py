...
from selectable.forms import AutoCompleteSelectField, AutoCompleteWidget
...
class SearchForm(forms.Form):
    ...
    officer = forms.CharField(label="Officer ID", required=False, help_text="ex: 227")
    start_date = forms.DateField(required=False, help_text="ex: 8/13/2013")
    end_date = forms.DateField(required=False, help_text="ex: 8/13/2014")
    age = forms.IntegerField(required=False, min_value=0, max_value=130)
    gender = forms.ChoiceField(
        required=False, initial=None, choices=addNoneOpt(stops.GENDER_CHOICES)
    )
    race = forms.ChoiceField(required=False, initial=None, choices=addNoneOpt(stops.RACE_CHOICES))
    ethnicity = forms.ChoiceField(
        required=False, initial=None, choices=addNoneOpt(stops.ETHNICITY_CHOICES)
    )
    purpose = forms.MultipleChoiceField(
        required=False, choices=stops.PURPOSE_CHOICES, widget=forms.CheckboxSelectMultiple
    )
    action = forms.MultipleChoiceField(
        required=False, choices=stops.ACTION_CHOICES, widget=forms.CheckboxSelectMultiple
    )
