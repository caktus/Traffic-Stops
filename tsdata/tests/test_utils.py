...
class TestUtils(TestCase):
    ...
    def test_get_csv_path(self):
        ...
        self.make_test_zip(zip_path)
        path_to_first_file_csv = get_csv_path("http://example.com/foo.zip", destination.name)
        self.assertEqual(
            os.path.join(destination.name, "file1-processed.csv"), path_to_first_file_csv
        )
        destination.cleanup()
    ...
    def test_get_datafile_path_with_url(self):
        ...
        zip_path = os.path.join(destination.name, "foo.zip")
        self.make_test_zip(zip_path)
        path_to_first_file = get_datafile_path("http://example.com/foo.zip", destination.name)
    ...
    def test_get_zipfile_path(self):
        self.assertEqual(
            "/tmp/favicon.ico", get_zipfile_path("https://example.com/favicon.ico", "/tmp")
        )
