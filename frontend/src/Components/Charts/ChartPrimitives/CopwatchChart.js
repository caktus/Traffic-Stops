...
function CopwatchChart({ children, ...props }) {
  return (
    <VictoryChart {...props} containerComponent={<VictoryVoronoiContainer />}>
      {children}
    </VictoryChart>
  );
}
...
export function CopwatchTooltip({
  yAxisLabel,
  transformCenter,
  pie,
...
const defaultTransformCenter = ({ x, y, datum }) => ({
  x: x + VORONOI_LABEL_X_OFFSET,
  y: y - datum.height,
});

export function CopwatchChartFlyout({
  yAxisLabel = defaultYAxisLabel,
  transformCenter = defaultTransformCenter,
  pie,
  tooltipFontSize,
  ...data
}) {
      ...
      >
        ...
        <S.DataList>
          {data.activePoints?.map((p) => (
            <S.DataListItem color={getColor(p)} fontSize={tooltipFontSize} key={getLabel(p)}>
              <S.DatumLabel>{getLabel(p)}:</S.DatumLabel>
              <S.DatumValue>{getValue(p)}</S.DatumValue>
            </S.DataListItem>
          )) || (
            <S.DataListItem fontSize={tooltipFontSize} color={getColor(data)} key={getLabel(data)}>
              <S.DatumLabel>{getLabel(data)}:</S.DatumLabel>
              <S.DatumValue>{getValue(data)}</S.DatumValue>
            </S.DataListItem>
          )}
        </S.DataList>
      </S.FlyoutContainer>
